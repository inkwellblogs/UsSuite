...
local function handleWeaponReload()
	...
	if isLobby then return end -- No HUD/refill in lobby
	...
	if weaponType == "Blades" then
		...
		-- 1. Refill Reserves from GasTank (if reserves empty)
		...
		-- 2. Equip Blade (if blade missing/transparent and reserves > 0)
	...
	elseif weaponType == "Spears" then
		...
		local spearsLabel = HUD:FindFirstChild("Main") and HUD.Main:FindFirstChild("Top")
			and HUD.Main.Top:FindFirstChild("Spears") and HUD.Main.Top.Spears:FindFirstChild("Spears")
		if not spearsLabel then return end

		local spearCount = tonumber(spearsLabel.Text:match("(%d+)%s*/")) or 0
